/**
 * Extract piece, from, to, and promotion from a move string. This should also parse invalid SAN like Pe2-e4, Rc1c4, Qf3xf7
 */
export const REGEXP_MOVE =
  /(?:(?:([pnbrqkPNBRQK])??([a-h]?[1-8]?)?x?-?([a-h][1-8])=?([qrbnQRBN])?)|O-O(?:-O)?)+([+#])?/

/**
 * Extract key and value from header in one match. Whitespace is tolerated
 * around the brackets and the key, since exporters emit e.g. [ Event "x" ].
 */
export const REGEXP_HEADER = /^\s*\[\s*([A-Z][A-Za-z]*)\s+"(.*)"\s*\]\s*$/

/**
 * Extract nag
 */
export const REGEXP_NAG = /([?!]+)/

/**
 * Match move number (with or without dots for compatibility)
 */
export const REGEXP_MOVE_NUMBER = /^[0-9]+\.{0,3}$/
