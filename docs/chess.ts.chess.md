...
|  [moveToUci(move)](./chess.ts.chess.movetouci.md) |  | <p>Long algebraic notation for a move played from the current position, using the castling encoding this instance's variant calls for.</p><p>Call it before playing the move, since it reads the current position to resolve castling.</p> |
